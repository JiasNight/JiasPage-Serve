...
public class SystemServiceImpl implements ISystemService {
  ...
  public List getRouterList() {
    ...
    try {
      List<Menu> mList = systemMapper.getRouterListByRole();
  ...
  public List<MenuVo> getMenuList() {
    List menuList = new ArrayList();
    try {
      List<Menu> mList = systemMapper.getMenuList();
      // lambda表达式实现List接口sort方法排序
      mList.sort(
          (m1, m2) -> {
            return m1.getOrder() - m2.getOrder();
  ...
  public boolean addMenuInfo(MenuVo menuInfo) {
    try {
      ...
      menu.setIsDeleted(0);
      int i = systemMapper.addMenuInfo(menu);
      if (i > 0) {
        return true;
      } else {
        return false;
  ...
  public boolean delMenuInfo(String mId) {
    try {
      int i = systemMapper.delMenuInfo(mId);
      if (i > 0) {
        return true;
      } else {
        return false;
